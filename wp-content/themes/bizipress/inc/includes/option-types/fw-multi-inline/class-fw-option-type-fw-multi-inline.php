...
class FW_Option_Type_FwMultiInline extends FW_Option_Type {
	...
	protected function _render( $id, $option, $data ) {
		return fw_render_view( BIZIPRESS_INC . '/includes/option-types/fw-multi-inline/view.php', array(
			'id'	 => $id,
			'option' => $option,
			'data'	 => $data
		) );
	...
	protected function _get_defaults() {
		...
		return array(
			...
			'fw_multi_options'	 => array(
				'firstoption' => array(
					'type'		 => 'select',
					'title'		 => esc_html__( 'Title', 'bizipress' ),
					'choices'	 => array(
						'select1'	 => esc_html__( 'select1', 'bizipress' ),
						'select2'	 => esc_html__( 'select2', 'bizipress' ),
						'select3'	 => esc_html__( 'select3', 'bizipress' )
					),
				)
			),
			'groupname'			 => ''

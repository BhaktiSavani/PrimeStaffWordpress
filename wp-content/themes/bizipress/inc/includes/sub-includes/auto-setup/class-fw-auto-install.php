...
class FW_Simple_Auto_Install {
		...
		if ( is_array( $result ) ) {
			...
			if ( ! empty( $failed_extensions ) ) {
				...
				wp_send_json_error( array(
					'message' => sprintf( esc_html__( 'Failed to activate supported extensions. %s ', 'bizipress' ), implode( ', ', $failed_extensions ) ),
				) );
			}
		}
	...
	public function get_setup_messages() {
		return array(
			'plugins_only'      => sprintf( esc_html__( 'This option will activate Unyson dependencies. The %s demo content will not be installed %s.', 'bizipress' ), '<b>', '</b>' ),
			'plugins_and_demo'  => sprintf( esc_html__( 'This option will activate Unyson dependencies together %s with the demo content %s for the theme.', 'bizipress' ), '<b>', '</b>' ),
			'skip_auto_install' => esc_html__( 'Skip the auto setup all together and activate all the Unyson dependencies manually. Note that this page will not be  accessible until you install the theme again.', 'bizipress' )
		);
	}
	...
	private function admin_enqueue_scripts() {
		...
		$auto_setup_data = array(
			...
			'messages'                  => array(
				'on_leave_alert'    => esc_html__( 'Attention, the installation process is not yet finished, if leave this page, you lose the information stored on the site!', 'bizipress' ),
				'server_problems'   => esc_html__( "Sorry, we've encountered some errors, try to access this page later.", "bizipress" ),
				'process_completed' => esc_html__( 'The installation process was completed successfully.', 'bizipress' )
			),
...
class FW_Admin_Auto_Install extends FW_Simple_Auto_Install {
		...
		if ( ! empty( $this->demos ) ) {
			...
			wp_localize_script(
				...
				array(
					...
					'messages'     => array(
						'installing'              => __( 'Installing', 'bizipress' ),
						'start_install_plugins'   => __( 'Installing required plugins ...', 'bizipress' ),
						'finish_install_plugins'  => __( 'Finished installing required plugins', 'bizipress' ),
						'start_activate_plugins'  => __( 'Activaiting required plugins ...', 'bizipress' ),
						'finish_activate_plugins' => __( 'Finished activaiting required plugins', 'bizipress' ),
					)
				)
			);
		}
	...
	public function get_demo_steps() {
		return array(
			'activate-demo-plugins' => array(
				...
				'message'     => esc_html__( 'Activate required demo plugins', 'bizipress' ),
			)
		);
	}
		...
		return array_merge(
			...
			array(
				'activate-unyson'           => array(
					...
					'message'     => esc_html__( 'Activating Unyson', 'bizipress' ),
				),
				'activate-required-plugins' => array(
					...
					'message'     => esc_html__( 'Activate required plugins', 'bizipress' ),
				)
			)
		);
	}
	...
	public function get_setup_messages() {
		return array(
			'plugins_only'      => sprintf( esc_html__( 'This option will activate %s already installed plugins and dependencies %s by Network Administrator. The %s demo content will not be installed %s.', 'bizipress' ), '<b>', '</b>', '<b>', '</b>' ),
			'plugins_and_demo'  => sprintf( esc_html__( 'This option will activate Unyson dependencies together %s with the demo content %s for the theme.', 'bizipress' ), '<b>', '</b>' ),
			'skip_auto_install' => esc_html__( 'Skip the auto setup all together and activate all the plugins manually. Note that this page will not be accessible until you install the theme again.', 'bizipress' )
		);
	}
	...
	public function activate_plugins( $plugins ) {
		if ( ! current_user_can( 'activate_plugins' ) ) {
			return new WP_Error( 0, __( 'Current user can\'t activate plugins', 'bizipress' ) );
		}
		...
		if ( ! empty( $failed_plugins ) ) {
			return new WP_Error( 0, sprintf(
					esc_html__( 'Failed to activate required plugins. %s', 'bizipress' ),
...
class FW_Super_Admin_Auto_Install extends FW_Admin_Auto_Install {
	...
	public function get_demo_steps() {
		return array(
			...
			'install-demo-plugins'  => array(
				...
				'message'     => esc_html__( 'Install required demo plugins', 'bizipress' ),
			),
		);
	}
	...
	public function install_demo_plugins() {
		...
		if ( ! $this->initialize_filesystem( $credentials, WP_PLUGIN_DIR ) ) {
			...
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to install required plugins. Folder %s is not writable', 'bizipress' ), WP_PLUGIN_DIR ),
			) );
		}
		...
		if ( ! empty( $failed_plugins ) ) {
			...
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to install required plugins. %s', 'bizipress' ), implode( ', ', $failed_plugins ) ),
			) );
		}
		ob_end_clean();
		wp_send_json_success();
		...
		return array_merge(
			...
			array(
				'install-unyson'               => array(
					...
					'message'     => esc_html__( 'Downloading and installing Unyson', 'bizipress' ),
				),
				'install-required-plugins'     => array(
					...
					'message'     => esc_html__( 'Installing required plugins', 'bizipress' ),
				),
				'install-supported-extensions' => array(
					...
					'message'     => esc_html__( 'Installing supported extensions', 'bizipress' ),
				),
				'install-child-theme'          => array(
					'ajax_action' => $this->_prefix . '_install_child_theme',
					'nonce'       => wp_create_nonce( 'install-child-theme' ),
					'message'     => esc_html__( 'Downloading and installing child theme', 'bizipress' ),
				),
			)
		);
	}
	...
	public function install_supported_extensions() {
		...
		if ( ! $this->initialize_filesystem( $credentials, WP_PLUGIN_DIR ) ) {
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to install supported extensions. Folder %s is not writable', 'bizipress' ), WP_PLUGIN_DIR ),
			) );
		}
		...
		if ( is_array( $result ) ) {
			...
			if ( ! empty( $failed_extensions ) ) {
				...
				wp_send_json_error( array(
					'message' => sprintf( esc_html__( 'Failed to install supported extensions. %s ', 'bizipress' ), implode( ', ', $failed_extensions ) ),
				) );
			}
		}
	...
	public function install_unyson() {
		...
		if ( ! $this->initialize_filesystem( $credentials, WP_PLUGIN_DIR ) ) {
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to install Unyson. Folder %s is not writable', 'bizipress' ), WP_PLUGIN_DIR ),
			) );
		}
	...
	public function install_child_theme() {
		...
		if ( ! $this->initialize_filesystem( $credentials, wp_normalize_path( get_theme_root() ) ) ) {
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to install Theme. Folder %s is not writable', 'bizipress' ), wp_normalize_path( get_theme_root() ) ),
			) );
		}
	...
	public function get_setup_messages() {
		return array(
			'plugins_and_demo'  => sprintf( esc_html__( 'This option will install and activate all the plugins and dependencies together %s with the demo content %s for the theme.', 'bizipress' ), '<b>', '</b>' ),
			'plugins_only'      => sprintf( esc_html__( 'This option will install and activate all the plugins and dependencies. The %s demo content will not be installed %s.', 'bizipress' ), '<b>', '</b>' ),
			'skip_auto_install' => esc_html__( 'Skip the auto setup all together and install all the plugins manually. Note that this page will not be accessible until you install the theme again.', 'bizipress' )
		);
	}
}
...
class FW_Plugin_Installer_Helper extends FW_Installer_Helper {
	...
	public static function install( $plugin, $force = false ) {
		return self::is_installed( $plugin ) && ! $force
			? array(
				'success' => true,
				'data'    => array(
					'message' => esc_html__( 'Plugin is already installed.', 'bizipress' )
				)
			)
	...
	public static function activate( $plugin ) {
		if ( ! self::is_installed( $plugin ) ) {
			return array(
				'success' => false,
				'data'    => array(
					'message' => sprintf( esc_html__( 'It was not possible to activate %s. Because it isn\'t installed.', 'bizipress' ), $plugin['name'] )
				)
			);
		}
	...
	public static function bulk_activate( $plugins ) {
		...
		if ( ! current_user_can( 'activate_plugins' ) ) {
			return array(
				'message' => sprintf( esc_html__( 'Current user can\'t activate plugins.', 'bizipress' ), WP_PLUGIN_DIR ),
			);
		}
	...
	private static function api( $action, $args = null ) {
		...
		if ( false === $res ) {
			...
			if ( is_wp_error( $request ) ) {
				$res = new WP_Error( 'plugins_api_failed', esc_html__( 'An unexpected error occurred. Something may be wrong with WordPress.org or this server&#8217;s configuration. If you continue to have problems, please try the <a href="https://wordpress.org/support/">support forums</a>.', 'bizipress' ), $request->get_error_message() );
			} else {
				$res = maybe_unserialize( wp_remote_retrieve_body( $request ) );
				if ( ! is_object( $res ) && ! is_array( $res ) ) {
					$res = new WP_Error( 'plugins_api_failed', esc_html__( 'An unexpected error occurred. Something may be wrong with WordPress.org or this server&#8217;s configuration. If you continue to have problems, please try the <a href="https://wordpress.org/support/">support forums</a>.', 'bizipress' ), wp_remote_retrieve_body( $request ) );
				}
			}

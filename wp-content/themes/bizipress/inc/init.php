...
class Bizipress_Theme_Includes {
	...
	public static function init() {
		...
		if ( class_exists( 'XsCustomPost\Xs_CustomPost' ) ) {
			...
			/**
			 *
			 * Service Custom Post And Taxonomies
			 * new XsCustomPost\Xs_CustomPost( 'Text Domain' )
			 * $tab->xs_init( 'URL', 'Singular', 'Plural', array( 'menu_icon' => 'dashicons-wp', 'supports' => array( 'title', 'editor','thumbnail','excerpt' ), 'rewrite'=> array('slug'=>'plural_dahes') ) );
			 * $tab_tax = new XsCustomPost\Xs_Taxonomies( 'bizipress' );
			 * $tab_tax->xs_init( 'service_cat', 'Service Category', 'Categories', 'service' );
			 */
			$tab = new XsCustomPost\Xs_CustomPost( 'bizipress' );
			$tab->xs_init( 'service', 'Service', 'Services', array( 'menu_icon'	 => 'dashicons-exerpt-view',
				'supports'	 => array( 'title', 'editor', 'thumbnail', 'excerpt' ),
				'rewrite'	 => array( 'slug' => 'service' ) ) );
		}

		if(class_exists('XsCustomPost\Xs_Taxonomies')){
